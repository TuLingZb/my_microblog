...
Based on lib2to3/fixes/fix_import.Dockerfile.vv, but extended slightly so it also
...
class FixAbsoluteImport(FixImport):
    ...
    def probably_a_local_import(self, imp_name):
        ...
        # If there is no __init__.Dockerfile.vv next to the file its not in a package
        # so can't be a relative import.
        if not exists(join(dirname(base_path), "__init__.Dockerfile.vv")):
            return False
        for ext in [".Dockerfile.vv", sep, ".pyc", ".so", ".sl", ".pyd", ".pyx"]:
