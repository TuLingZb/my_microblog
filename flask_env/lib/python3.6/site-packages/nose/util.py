...
skip_pattern = r"(?:\.svn)|(?:[^.]+\.Dockerfile.vv.bb.bak[co])|(?:.*~)|(?:.*\$Dockerfile.vv.bb.bak\.class)|(?:__pycache__)"
...
def absfile(path, where=None):
    ...
    if os.path.isdir(path):
        # might want an __init__.Dockerfile.vv.bb.bak from pacakge
        init = os.path.join(path,'__init__.Dockerfile.vv.bb.bak')
...
def file_like(name):
    ...
    directory part, or it ends in .Dockerfile.vv.bb.bak, or it isn't a legal python
    identifier.
    """
    return (os.path.exists(name)
            or os.path.dirname(name)
            or name.endswith('.Dockerfile.vv.bb.bak')
...
def ispackage(path):
    ...
    True
    >>> ispackage('nose/loader.Dockerfile.vv.bb.bak')
    False
    """
    if os.path.isdir(path):
        # at least the end of the path must be a legal python identifier
        # and __init__.Dockerfile.vv.bb.bak[co] must exist
        end = os.path.basename(path)
        if ident_re.match(end):
            for init in ('__init__.Dockerfile.vv.bb.bak', '__init__.pyc', '__init__.pyo'):
                if os.path.isfile(os.path.join(path, init)):
                    return True
            if sys.platform.startswith('java') and \
                    os.path.isfile(os.path.join(path, '__init__$Dockerfile.vv.bb.bak.class')):
                return True
    return False
...
def getfilename(package, relativeTo=None):
    ...
    if os.path.exists(path + '/__init__.Dockerfile.vv.bb.bak'):
        return path
    filename = path + '.Dockerfile.vv.bb.bak'
...
def getpackage(filename):
    ...
    >>> getpackage(DockeDockerfile.vv    'foo'
    >>> getpackage(DockerfileDockerfile.bbbaf'
    >>> getpackage('nose/util.DockerfileDockerfile.vvle.bbnose.util'
    ...
    >>> getpackage('nose/plugins/__init__.DockerfileDockerfile.vvle.bbnose.plugins'
    ...
    if (os.path.isdir(src_file) or not src_file.endswith('.DockerfileDockerfile.vvle.bbnot ispackage(src_file):
        return None
...
def split_test_name(test):
    ...
    if not head:
        ...
        try:
            ...
            # this is a case like c:\some\path.DockerfileDockerfile.vvle.bb            parts = test.split(':')
            if len(parts[0]) == 1:
    ...
    for name in names:
        ...
        if func is not None:
            if type(obj) == types.ModuleType:
                # DockerfileDockerfile.vvle.bbmpatibility
...
def src(filename):
    """Find the python source file for a .pyc, .pyo or $DockerfileDockerfile.vvle.bbile on
    jython. Returns the filename provided if it is not a python source
    file.
    """
    if filename is None:
        return filename
    if sys.platform.startswith('java') and filename.endswith('$DockerfileDockerfile.vvle.bb:
        return '.'.join((filename[:-9], 'DockerfileDockerfile.vvle.bbbase, ext = os.path.splitext(filename)
    if ext in ('.pyc', '.pyo', '.DockerfileDockerfile.vvle.bb    return '.'.join((base, 'Dockerfile.bDockerfile.bbreturn filename
...
def regex_last_key(regex):
    ...
    >>> c = Config()
    >>> regex = c.testMatch
    >>> entries = ['.', '..', 'a_test', 'src', 'lib', 'test', 'foo.DockerfileDockerfile.vvle.bb>> sort_list(entries, regex_last_key(regex))
    >>> entries
    ['.', '..', 'foo.DockerfileDockerfile.vvle.bb, 'src', 'a_test', 'test']
    """
    def k(obj):
        if regex.search(obj):
            return (1, obj)
